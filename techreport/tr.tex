...
\acmConference[RTKS '25]{Real-Time Kernels and Systems Exam}{19/09/2025}{Padova, Italy}
%%
...
\begin{abstract}
  ...
  uses a \textit{Fixed-Priority Scheduling} policy combined with variants of
  \textit{Priority Ceiling Protocol (PCR)} for resource sharing. 
...
\begin{table}[h]
  ...
  \resizebox{\linewidth}{!}{%
    \begin{tabular}{lc}
      \toprule
      \textbf{Description of changes} & \textbf{Reference} \\
      \midrule
      Improve Whetstone explanation.                                            & \ref{lab:whetstone-explanation} \\
      Improve Deadline Watchdog task description.                               & \ref{lab:deadline-watchdog-task} \\
      Provide harmony definition.                                               & \ref{lab:harmony-definition} \\ 
      Improve RTIC properties.                                                  & \ref{lab:rtic-properties} \\
      Improve explanation regarding why SRP is not used in general-purpose OS.  & \ref{lab:srp-in-general-purpose-os} \\
      Provide explanation about compiler checks on await operations while
      holding a resource.                                                       & \ref{lab:compiler-checks-on-await} \\
      Provide explanation about differences in WCET between Ada and Rust for
      Whetstone operation.                                                      & \ref{lab:wcet-differences-ada-rust} \\
      Improve priority assignment explanation.                                  & \ref{lab:priority-assignment} \\
      Improve the explanation of the 90 increase factor.                        & \ref{lab:90-increase-factor} \\
      Improve the description of the optimization problem model.                & \ref{lab:optimization-problem-model} \\
      Explanation of the discrepancy between the sensitivity analysis and the
      offset-based results.                                                     & \ref{lab:exp-offset-sensitivity-discussion} \\
      Conclusions regarding shared resource management in different systems.    & \ref{lab:shared-resource-management-conclusions} \\
      \bottomrule
    \end{tabular}
  }
  \caption{Change Log}
