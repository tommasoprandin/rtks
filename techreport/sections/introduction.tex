...
its desired behaviour through runtime mechanisms. One of the most
...
Ravenscar Profile} is re-implemented in \textit{Rust} using the
\textit{Real-Time Interupput-driven Concurrency (RTIC)} framework \cite{RTIC},
which is responsible for triggering the scheduling of tasks through a
\textit{Fixed-Priority Scheduling} and the resource sharing in the system under
\textit{Stack Resource Policy (SRP)}. \\ 
For the analysis of the system, the \textit{MAST} tool
\cite{MASTComprehensiveTool} is used to verify the timing properties and to
analyze the system under different workloads, with the system running on a
\textit{STM32F4} microcontroller. \\  
