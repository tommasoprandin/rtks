...
runtime components for the system, paying attention to select compatible components
that can work in harmony together i.e. with compatible APIs and functioning in the chosen runtime environment. In our case this means choosing a
...
\begin{itemize}
  \item Bounded priority inversion blockage by the longest critical section of a lower priority job;
  \item Absence of circular-wait deadlocks;
  \item Tasks may utilize a single shared stack;
  \item Tasks are guaranteed to have wait-free access to all the resources they need after release;
\end{itemize}
...
generally not possible. In addition to this, the protocol has a runtime cost associated to every resource access and release, that involves updating the current system ceiling $\pi_s(t)$; and also at every job release to compare its priority against the system ceiling.
...
two yield points can be seen as an individual task. The compiler enforces that no \textit{await} operations can occur while holding a resource, preventing violations of the strict LIFO requirement on resource usage mandated by SRP \cite{RticrsRtic2025}. However, this compile-time guarantee only applies to statically analyzed code; externally compiled procedures can bypass this verification. The support for this style of programming simplifies the development of complex tasks by automatically managing state transitions between suspension points, eliminating the need for manual state machine implementations when handling multi-stage operations such as inter-task communication and hardware interactions.
\subsection{Defmt}
