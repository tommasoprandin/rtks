...
obtain the values that make the system fail to meet its deadlines
...
after execution of 50 \textit{hyperperiods}, which was considered to be a sufficient number of tests. 
...
Nonetheless, results still follow a consistent behavior when overload is reached, with the lowest priority jobs suffering from interference and incurring in deadline misses.
...
The code relevant for this work is available on GitHub repositories, the URLs can be found in the references section. The \textit{rtks} repo \cite{prandinTommasoprandinRtks2025} contains the Rust system code, while the \textit{gee} repo \cite{prandinTommasoprandinGee2025} contains the Ada one. The identification of \textit{RTIC} system overheads was performed in the \textit{Profiling-RTIC} repository \cite{bresolinGianlucaBresolinProfilingRTIC2025}.
