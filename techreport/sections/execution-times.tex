...
In particular, we have observed a very large difference in the execution times of the Whetstone procedure in favor of the Ada implementation. For the Rust implementation we have used a math library (libm), which is purely written in Rust and meant to become the reference implementation for the Rust compiler. At the current development stage though, it is not very optimized and more intended to be used as a fallback, rather than the actual library.
On the other hand Ada (in its GNAT distribution) has a more mature and optimized implementation, which has been tested on the field for many years now.

This may partially explain the large gap in execution times between the two systems.
