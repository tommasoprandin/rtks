...
Obtaining the \textit{Worst-Case Execution Times} for every operation identified in the analysis of the
...
analysis in order to prevent any interference that could invalidate our time measurements.
...
NOTE: How do we explain the difference between rust and ada in execution whetstone? I think we should say that we used a very basic math library for Rust, only meant to be used as a fallback (libm) and not yet stable even. We used it to avoid including an external C library which would have complicated the compilation phase. While Ada uses a more optimized math library. I don't know if you have better ideas.
