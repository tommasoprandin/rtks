...
by the use of the \textit{Small Whetstone} algorithm to simulate computational workloads. Thanks to its configurable load and deterministic behavior without involving
...
If the offset-based analysis helps in reducing the interference between tasks, we increase the workloads beyond what was achieved with the holistic analysis. The reason is that the critical instant
faced in the holistic analysis is similar to the one faced with the offset-based
analysis in the 12th period, when the \texttt{regualr\_producer} activates both 
...
entire available time slot. Since this is an unrealistic scenario, we chose to assign the minimum workload
