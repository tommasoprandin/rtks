...
Priority assignment was done according to the \textit{Deadline Monotonic}
scheme, which assigns higher priorities to tasks with shorter deadlines. This
choice comes from the fact that tasks in the system do not have implicit
deadlines and, as theory proves, \textit{Deadline Monotonic} performs better
than \textit{Rate Monotonic} in such cases, as it can provide feasible schedules
where \textit{Rate Monotonic} fails.
...
independent. As a consequence, this analysis is pessimistic, since it does not
take into account any relationship between tasks. System feasibility is
...
If the offset-based analysis helps in reducing the interference between tasks,
...
period, when the \texttt{regualr\_producer} activates both the
...
entire available time slot. Since this is an unrealistic scenario, we chose to
assign the minimum workload possible to the \texttt{on\_call\_producer}, i.e., a
workload equal to a single Whetstone operation. 
The remaining time in $T$ is then assigned to the
\texttt{activation\_log\_reader}, resulting in 0.49948648 - 0.00004456 =
0.49944192 seconds for the Rust system and 0.49805977 - 0.00002079 =
0.49785182 seconds for the Ada system. Here, 0.00004456 and 0.00002079 correspond
to the WCETs of the \texttt{on\_call\_producer} in \textit{Rust} and \textit{Ada},
respectively, which include not only the execution of a single Whetstone
operation but also all the other operations that the job must normally perform. 
